...
if [ -z "$ASSISTANT_ARN" ] || [ -z "$MODEL_ID" ] || [ -z "$REGION" ] || [ -z "$LOCALE" ] || [ -z "$AGENTS_JSON" ] \
   || [ -z "$PROMPTS_JSON" ] || [ -z "$ENV" ] || [ -z "$OUTPUT_PATH" ]; then
...
delete_unused_prompts() {
  ...
  ACTION=$1
  echo "=================================================="
  echo "Checking for unused prompts to delete...(ACTION : $ACTION)"

  local jq_condition
  if [ $ACTION == 'upsert' ]; then
    jq_condition='.[] | select(.use_flag == "false")'
  else
    jq_condition='.[]'
  fi

  echo "$PROMPTS_JSON" | jq -c "$jq_condition" | while read -r prompt_obj; do
    local prompt_name
...
delete_unused_agents() {
  ...
  ACTION=$1
  echo "=================================================="
  echo "Checking for unused agents to delete...(ACTION : $ACTION)"
  ...
  echo "$AGENTS_JSON" | jq -c "$jq_condition" | while read -r agent_obj; do
    local agent_name
    agent_name=$(echo "$agent_obj" | jq -r '.agent_name')
    agent_type=$(echo "$agent_obj" | jq -r '.agent_type')
...
# 메인 로직
# ==============================================================================
upsert_prompt_and_agent() {
  # 1. 미사용 에이전트, 프롬프트를 먼저 삭제합니다.
  ACTION=$1
  delete_unused_prompts $ACTION
  delete_unused_agents $ACTION
  ...
  jq -n \
    --arg ansgen_prompt_id_version "$ansgen_prompt_id_version" \
    --arg preproc_prompt_id_version "$preproc_prompt_id_version" \
    --arg self_service_agent_id_version "$self_service_agent_id_version" \
    '{ansgen_prompt_id_version: $ansgen_prompt_id_version, preproc_prompt_id_version: $preproc_prompt_id_version, self_service_agent_id_version: $self_service_agent_id_version}' > "$OUTPUT_PATH"
...
remove_assistant_ai_agent() {
  ...
  echo "=================================================="
  echo "Remove Assistant AI Agent(default agent) : AGENT_ID $EXISTING_AGENT_ID, AGENT_TYPE $AGENT_TYPE"

  aws qconnect remove-assistant-ai-agent \
    --assistant-id $EXISTING_AGENT_ID \
    --ai-agent-type $AGENT_TYPE > /dev/null

  echo "Remove Assistant AI Agent complete"
  echo "=================================================="
}
...
case "$ACTION" in
  upsert)
    upsert_prompt_and_agent $ACTION
    ;;
  ...
  *)
    echo "Error: Invalid action '$ACTION'. Use 'upsert' or 'delete'." >&2
    exit 1
    ;;
esac
