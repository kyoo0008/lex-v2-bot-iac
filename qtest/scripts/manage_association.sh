...
if [ -z "$CONNECT_INSTANCE_ID" ]; then
  echo "Error: CONNECT_INSTANCE_ID environment variable is not set."
  exit 1
fi

# --- 공통 함수: 특정 타입의 연결을 모두 삭제 ---
# $1: 삭제할 IntegrationType (예: WISDOM_ASSISTANT)
delete_associations_by_type() {
  local INTEGRATION_TYPE="$1" # 함수 내 지역 변수로 선언

  echo "Searching for associations of type: $INTEGRATION_TYPE"
...
case "$ACTION" in
  ...
  create)
    ...
    delete_associations_by_type "WISDOM_ASSISTANT"
    delete_associations_by_type "WISDOM_KNOWLEDGE_BASE"
    create_association "WISDOM_ASSISTANT" "$ASSISTANT_ARN"
    create_association "WISDOM_KNOWLEDGE_BASE" "$KB_ARN"
    ;;
