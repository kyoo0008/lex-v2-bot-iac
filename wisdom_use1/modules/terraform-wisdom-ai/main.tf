...
resource "terraform_data" "wisdom_ai_manager" {
  ...
  triggers_replace = {
    script_hash = filemd5("${path.module}/scripts/manage_wisdom_ai.sh")
    model_id    = var.prompt_model_id
    agent_configs = jsonencode(local.agent_configs)
    assistant_arn       = var.assistant_arn
    output_path         = local.output_file_path
    prompt_configs      = local.prompt_configs
  }
  ...
  provisioner "local-exec" {
    command = "chmod +x ${path.module}/scripts/manage_wisdom_ai.sh && ${path.module}/scripts/manage_wisdom_ai.sh upsert"
...
resource "terraform_data" "wisdom_ai_destroy_manager" {
  ...
  provisioner "local-exec" {
    when = destroy
    command = "chmod +x ${path.module}/scripts/manage_wisdom_ai.sh && ${path.module}/scripts/manage_wisdom_ai.sh delete"
